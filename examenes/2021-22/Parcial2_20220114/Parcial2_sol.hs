...
dropWhileArbol p (H x) | (not.p) x = [H x]
                       | otherwise = []
...
ejArbolRN :: RedBlackTree Int
ejArbolRN = (Black 11 (Red 2 (Black 1 Nil Nil) (Black 7 (Red 5 Nil Nil) (Red 8 Nil Nil))) (Black 14 Nil (Red 15 Nil Nil)))
...
softmax p = listArray b [soft i j | (i,j) <- indices p]
  where soft i j = (exp 1)**(p!(i,j)) / sumaSoft j
...
normalizada p =
  and [ abs (1-sum [ p!(i,j) | i <- [1..nf]]) < 0.001 | j <- [1..nc] ]
   where b = bounds p
         nf = fst (snd b)
         nc = snd (snd b)
...
parser' :: String -> Pila Char -> Bool
parser' [] p = esVacia p 
parser' (c:s) p
  | c == '(' || c == '{' = parser' s (apila c p)
  | c == ')' && cima p == '(' = parser' s (desapila p)
  | c == '}' && cima p == '{' = parser' s (desapila p)
  | c == ')' || c == '}' = False
  | otherwise = parser' s p
